...
RUN apt-get update \
  && apt-get install -y --no-install-recommends build-essential curl imagemagick git libpq-dev  \
...
RUN apt-get update \
  && apt-get install -y --no-install-recommends build-essential curl imagemagick libpq-dev \
  && rm -rf /var/lib/apt/lists/* /usr/share/doc /usr/share/man \
  && apt-get clean \
  && useradd --create-home ruby \
